...
async function main() {
    ...
    const routes = getPermutations(locations.length).map(routes => [0, ...routes]);
    ...
    const routeStats = routes.map((route) => {
        ...
        const stats = segments.reduce((acc, [origin, destination]) => {
            const {distance, duration} = dMatrix.rows[origin].elements[destination];
            acc.distance += distance.value;
            acc.duration += duration.value;
            return acc;
        }, {distance: 0, duration: 0});
        return {
            ...stats, route: route.map(i => locations[i]), segments
        }
    });
    const compare = (measurement) => (a, b) => a[measurement] - b[measurement];
    const shortestDurationRoute = routeStats.sort(compare('duration'))[0];
    console.log(`Shortest Time Routing: ${shortestDurationRoute.duration} seconds`);
    console.log(shortestDurationRoute.route.map((v, i) => `${i + 1}. ${v}`).join('\n'));
}
